...
#define load(m)      _mm512_loadu_si512((const __m512i *)(m))
#define store(m,a)   _mm512_storeu_si512((__m512i *)(m),a)
#define xor(a,b)     _mm512_xor_si512(a,b)
#define and(a,b)     _mm512_and_si512(a,b)
#define enc(a,k)     _mm512_aesenc_epi128(a,k)
#define setzero()    _mm512_setzero_si512()
#define extract(a,i) _mm512_extracti32x4_epi32(a,i)
...
#define P_NUM      ( 4)
#define S_NUM      ROCCA_SX4_STATE_NUM
#define M_NUM      ( 2)
#define INIT_LOOP  (16)
#define TAG_LOOP   (16)
#define MSGBLKSIZE ROCCA_SX4_MSG_BLOCK_SIZE

#define DECL_VARS_4_UPDATE \
  __m512i state[S_NUM], stateNew[S_NUM], M[M_NUM];

#define DECL_VARS_4_INIT \
  DECL_VARS_4_UPDATE; __m512i k[2];
  
#define DECL_VARS_4_ENCRYPT \
  DECL_VARS_4_UPDATE; __m512i Z[M_NUM], C[M_NUM];
...
static __m512i broadcast(const uint8_t * data16bytes) {
...
static __m512i broadcast_with_index(const uint8_t* data16bytes) {
	uint8_t tmp[16 * P_NUM];
	for (size_t i = 0; i < P_NUM; ++i) {
		memcpy(tmp + 16 * i, data16bytes, 16);
		*(tmp + 16 * i) ^= i;
...
#define MAKE_TAG(sizeAD, sizeM, tag) \
  ...
  __m512i tag512a = setzero(); \
  for(size_t i = 0; i <= 3; ++i) { \
    tag512a = xor(tag512a, state[i]); \
  } \
  __m512i tag512b = setzero(); \
  for(size_t i = 4; i <= 6; ++i) { \
    tag512b = xor(tag512b, state[i]); \
  } \
  __m128i tag128[2] = {setzero128(), setzero128()}; \
  for(size_t i = 0; i < P_NUM; ++i) { \
    tag128[0] = xor128(tag128[0], extract(tag512a, i)); \
    tag128[1] = xor128(tag128[1], extract(tag512b, i)); \
  } \
